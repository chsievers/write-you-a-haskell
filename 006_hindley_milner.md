...
*** that looks like explicitly typed lambda calculus
\end{aligned}
$$
...
*** That sentence ends prematurely
...
\end{aligned}
$$
*** We added that definition to a previous chapter.
...
```
*** Storing the infinite list of unused strings is nearly as obvious and
*** avoids using (!!). I'll show in my 'experiments' branch.
...
& \quad \trule{Uni-Con} \\ \\
*** our language has no types of that form
\infrule{
  \tau_1 \sim \tau_1' : \theta_1 \andalso [\theta_1] \tau_2  \sim [\theta_1] \tau_2' : \theta_2
}{
...
*** My first idea would be to use a Set
...
inEnv (x, sc) m = do
  ...
  local scope m
```
*** Because of the way extend is defined the preceding remove is not needed.
...
*** No input can result in this error, it could only occur as an internal error.
*** In fact this function is only ever called with lists of length two
*** (and recursively).
```
